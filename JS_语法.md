- [js的基本语法](#js的基本语法)
  ...
  - [label语句](#label语句)
    - [与break一起使用](#与break一起使用)
    - [与continue一起使用](#与continue一起使用)
    - [与代码块一起使用](#与代码块一起使用)
...
* 在javascript中所有表达式都有返回值(如果没有返回值就是undefined)，这个返回值就可以继续作为表达式的一部分。大多数语言表达式的定义为　　
　　
1. 字面量(例如1，2，3，'abc')　
...
=======
```
>>>>>>> 74f1afb73fc172c56b4a9d2d7d2b76af7bca5c59
　　1a //第一个字符不能为数字
　　a b  // 不能包含空格
　　a@b //包含特殊符号
　　while  //关键字
...
```javascript
    ...
    console.log(i);//10
```
　　在第一个for循环中,当外循环执行到第三次,即i=2的时候,break将内层for循环结束;i继续增加;所以最后i一直增加到10;在第二个for循环中,使用label语句将外层循环添加标记"outFor",当执行到i=2时,break outFor;实现将外层循环结束.所以i为2.也就实现了双重for循环中,在内层循环结束掉外层循环.弥补了单纯的break只能结束所在for循环的不足.　
### 与continue一起使用
```javascript
    for(var i=0;i<10;i++){
        for(var j=0;j<10;j++){
